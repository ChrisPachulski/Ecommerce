...
h2o_dl_workflow = workflow() %>%
  add_model(model_spec_dl_h2o) %>%
  add_recipe(recipe_spec_dl) %>%
  fit(training(splits))

h2o_dl_workflow %>% automl_leaderboard()

modeltime_tbl = modeltime_table(h2o_workflow,h2o_dl_workflow) %>% 
  update_model_description(.model_id = 1,.new_model_desc ="TRADITIONAL")%>% 
  update_model_description(.model_id = 2,.new_model_desc ="DEEPLEARNING")  %>%
  ensemble_average(type = "median") %>% modeltime_table()

calibration_tbl = modeltime_tbl %>% modeltime_calibrate(testing(splits))

calibration_tbl %>% modeltime_accuracy()


forecast_ensemble_tbl = calibration_tbl              %>% 
...
rmse_to_keep = metrics %>% arrange(rmse) %>% mutate(rank = seq(nrow(.)), tier = ntile(rmse,10) )%>% filter(tier <= 3) %>% select(-tier)
...
model_ensemble_refit_tbl = calibration_tbl %>% 
...
boxplot_overview_tbl = raw_query %>% group_by(Key) %>% summarize(iqr = IQR(BL), range = fivenum(BL), sd = sd(BL)) %>% ungroup() %>%
  left_join(expanded_all_forecasts %>% filter(!grepl("ACTUAL",.model_desc)) %>% group_by(Key) %>% filter(Date > (Sys.Date()+ 5) ) %>%
              mutate(max_forecast_value = max(.value), max_date = ifelse(max_forecast_value == .value, Date,NA)) %>% #filter(Date == max(Date)) %>% 
              select(-BL,-set_release) %>% drop_na() %>%
              select(Key, max_forecast_value,Date), by = c("Key"="Key")) %>% drop_na() %>%
  ...
  filter(Date > (Sys.Date()+ 5) ) %>%
  mutate(current_val = BL,plus_minus = mae) %>% select(-BL,-mae) %>% 
  select(Key,current_val,iqr,range,sd,max_forecast_value,plus_minus,Date) %>% distinct()

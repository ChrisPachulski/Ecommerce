...
} #Character Count utilization of 'left'&'right' functions for quantity breakdown ~ Outdated via dplyr ~
...
gaeas_cradle <- function(email){
  ...
  options(scipen = 20)
  con
} #Connect to personalized BQ DB for exports and storage
tracker_creation <- function(tracker){
  ...
  tracker
} #Necessary for metric aggregation, compiles daily files into workable base for analysis and future ts forecasts
prior_3_weeks <- function(tracker){
  MBT <- tracker[,-c(1:5)]
  ncol(MBT)
...
ck_tcg_comparison <- function(CK_Retail_Comparison){
...
} #Needed for final market comparison
#Packages####
Packages <- c("plyr","dplyr","tidyverse","rvest","jsonlite","devtools","googlesheets4","googledrive","bigrquery","gargle","httr")
invisible(lapply(Packages, library, character.only = TRUE))
#Refresh Our Master Roster via mtgjson API ~ Spoiler Season will haunt us no longer! ~ ####
...
#CK Buylist ~ Ping Cardkingdom API for their standing Offers####
...
#Goldfish Pricing Retrieval ~ Relatively inaccurate, acts as our glue for NA's ~#### 
...
#CK Best Sellers - best seller list has no api, so, we do it the hard way with parallel processing & scraping####
library(foreach)
library(doParallel)
...
#TCG Market - Recreate their expected json packet for site query, iterate for top 10K best sellers (max allowed)####
Start_Time <- Sys.time()
A <- 0
B <- 100
C <- 100
...
Pricing %>% mutate(CK_Rank = ifelse(CK_Rank == 0,Worst_CK_Rank,CK_Rank)) %>% arrange(CK_Rank) %>% mutate(CK_Rank = seq(nrow(Pricing)))
...
premium_bq_export <- Final_Export[c(1:12)] %>% rename(c("Foil_Status" = "F/NF")) %>% mutate(Date = currentDate) 
...
premium_bq_export <- premium_bq_export %>% mutate(Set = ck_conversion$Standardized[match(Set,ck_conversion$CK)]) %>% 
                     mutate(Key = paste(Card,Set,Rarity," ",Foil_Status,sep="")) %>% mutate(BL_QTY = as.numeric(as.character(BL_QTY))) %>%
...
tcgplayer_bq_export <- TCG_Vendor %>% rename(c("Vendors" = "Vendor Listings")) %>% mutate(Vendors = as.numeric(Vendors)) %>% select(Product_ID,MKT_EST,Vendors,MKT,Rank,Direct_Listings,Potential_Direct_Copies,Total_Copies)
...
#Funny Money Report - Vendor offers 30% credit on the dollar for `in-store only money`, hence the nicknbame of `funny money`####
...
Consistent_BuyLists_Export <- Consistent_BuyLists %>%
  mutate(Param = Updated_Tracking_Keys$param[match(Key,Updated_Tracking_Keys$Key)]) %>% select(!c(Name,Set,Rarity,Foil)) %>% select(Param, everything()) 
...
Consistent_Vendors_Export <- Consistent_Vendors %>%
  mutate(Param = Updated_Tracking_Keys$param[match(Key,Updated_Tracking_Keys$Key)]) %>% select(!c(Name,Set,Rarity)) %>% select(Param, everything()) %>% rename(c("Key" = "Unique_Keys"))
...
Market_Comparison <- data.frame(All = AT_CK$Key,Three_Week = Three_Week_CK$Key,One_Week = One_Week_CK$Key) %>% gather() %>% select(value) %>% distinct() %>% rename(c("Key" = "value")) %>%

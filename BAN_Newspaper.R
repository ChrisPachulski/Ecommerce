...
sets = Sets %>% select(sets) %>% filter(sets != "" & sets != "BOOSTER" & sets != "DDR" & sets != "PD2" & sets != "PD3", 
                                        ...
                                        sets != "PRM-SDCC18" & sets != "PRM-SDCC19" & sets != "P2XM") #%>% as.list()
total <- nrow(sets)
...
for(i in 1:total){
  ...
  Gold <- "scrapedpage.html" %>% read_html()%>%html_nodes("table") %>% .[2] %>% html_table(fill = TRUE) %>% as.data.frame() 
...
sets_Foil = Sets %>% select(sets_Foil) %>% filter(sets_Foil != "" & sets_Foil != "DST_F" & sets_Foil != "OGW_F" & sets_Foil != "VI_F" &
                                                  sets_Foil != "MB1_F" & sets_Foil != "UGL_F") #%>% as.list() %>% as.character()
...
for(i in 1:total){
  ...
  dld_url <- download.file(url, destfile = "scrapedpage.html", quiet=TRUE)
  Shiny_Gold <- "scrapedpage.html" %>% read_html()%>%html_nodes("table") %>% .[2] %>% html_table(fill = TRUE) %>% as.data.frame()
...
CK_Prices_df <- CK_Prices_df %>% arrange((Rank)) %>% mutate(Rank = seq(nrow(CK_Prices_df)), 
                                                            Card = trimws(gsub("\n"," ",Card)),
                                                            key = trimws(paste(Card,Set,Rarity,sep=""))) %>% select(Card, Set, Rarity, Price, key, Rank)
...
TCG_Export <- TCG_Export %>% mutate(Set = Sets_V2$mtgjson[match(Set,Sets_V2$TCG_Key)], number = Updated_Tracking_Keys$number[match(Product_ID,Updated_Tracking_Keys$param)]) %>% mutate(Primary_Key = paste(Card_Name,Set, Rarity, sep = ""))

TCG_Export$number_key = paste(Updated_Tracking_Keys$abbr[match(TCG_Export$Product_ID,Updated_Tracking_Keys$param)],"-",TCG_Export$number,sep="")
...
CK_Smaller_List <- CK_Smaller_List %>% mutate(CK_Key = trimws(CK_Key)) %>% mutate(TCG_Rank = TCG_Export$Rank[match(param,TCG_Export$Product_ID)]) %>%
                    mutate(TCG_Price = TCG_Export$MKT_EST[match(number_key,TCG_Export$number_key)]) %>% replace_na(list(Gold_Market = 0)) 
...
Pricing <- Pricing %>% mutate(CK_Rank = ifelse(CK_Rank == 0,Worst_CK_Rank,CK_Rank)) %>% arrange(CK_Rank) %>% mutate(CK_Rank = seq(nrow(Pricing)),
                                                                                                                    param = Updated_Tracking_Keys$param[match(ckid,Updated_Tracking_Keys$ckid)])

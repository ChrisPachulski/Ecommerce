...
unique_card = raw_list %>% #filter(Key == ukey) %>% 
    ...
    #left_join(lagged_raw_query,by=c("dated_key" = "dated_key")) %>% select(-dated_key)
...
full_tbl = unique_card                                   %>%
    ...
    map(.f = function(df){
        df %>% 
            ...
            tk_augment_lags(BL         , .lags = c(14,17,21,25,28))   %>%
            
            tk_augment_lags(BL_QTY     , .lags = c(14,17,21,25,28))   %>%
            
            tk_augment_lags(MKT        , .lags = c(14,17,21,25,28))   %>%
            
            tk_augment_lags(Sellers    , .lags = c(14,17,21,25,28))   %>%
            
            tk_augment_lags(TCG_Rank   , .lags = c(14,17,21,25,28))   %>%
            
            tk_augment_lags(CK_ADJ_Rank, .lags = c(14,17,21,25,28))   %>%
            ...
            tk_augment_slidify(BL_QTY_lag21              ,
                               .f       = ~mean(.x,na.rm=T),
                               .period  = c(14,28,56)  ,
                               .partial = T          ,
                               .align   = "right")          %>%
            tk_augment_slidify(BL_QTY_lag28              ,
                               .f       = ~mean(.x,na.rm=T),
                               .period  = c(14,28,56)  ,
                               .partial = T          ,
                               .align   = "right")          %>% 
            ...
            tk_augment_slidify(MKT_lag21              ,
                               .f       = ~mean(.x,na.rm=T),
                               .period  = c(14,28,56)  ,
                               .partial = T          ,
                               .align   = "right")          %>%
            tk_augment_slidify(MKT_lag28              ,
                               .f       = ~mean(.x,na.rm=T),
                               .period  = c(14,28,56)  ,
                               .partial = T          ,
                               .align   = "right")          %>%
            ...
            tk_augment_slidify(Sellers_lag21              ,
                               .f       = ~mean(.x,na.rm=T),
                               .period  = c(14,28,56)  ,
                               .partial = T          ,
                               .align   = "right")          %>%
            tk_augment_slidify(Sellers_lag28              ,
                               .f       = ~mean(.x,na.rm=T),
                               .period  = c(14,28,56)  ,
                               .partial = T          ,
                               .align   = "right")          %>%
            ...
            tk_augment_slidify(TCG_Rank_lag21              ,
                               .f       = ~mean(.x,na.rm=T),
                               .period  = c(14,28,56)  ,
                               .partial = T          ,
                               .align   = "right")          %>%
            tk_augment_slidify(TCG_Rank_lag28              ,
                               .f       = ~mean(.x,na.rm=T),
                               .period  = c(14,28,56)  ,
                               .partial = T          ,
                               .align   = "right")          %>%
            ...
            tk_augment_slidify(CK_ADJ_Rank_lag21              ,
                               .f       = ~mean(.x,na.rm=T),
                               .period  = c(14,28,56)  ,
                               .partial = T          ,
                               .align   = "right")          %>%
            tk_augment_slidify(CK_ADJ_Rank_lag28              ,
                               .f       = ~mean(.x,na.rm=T),
                               .period  = c(14,28,56)  ,
                               .partial = T          ,
                               .align   = "right")          
...
models_to_keep_ensemble = model_tuned_resample_tbl      %>% 

...
statement <- paste(
  "SELECT a.Key,a.Card,a.Set,a.Rarity,a.Foil_Status,a.BL,a.Date ",
...
raw_query_base$day <- raw_query_day_one$BL[match(raw_query_base$Key,raw_query_day_one$Key)]
...
for (i in 1:90){
  ...
  raw_query_base$day <- rqb$BL[match(raw_query_base$Key,rqb$Key)]
  colnames(raw_query_base)[ncol(raw_query_base)] <- format(as.Date(timeframe), "%m/%d/%Y")
  timeframe <- as.Date(timeframe) + 1
}
...
for(Key in Reformatted_Keys[c(1:total),]){
  ...
  bl_train_data <- window(Buy_List_ts_final[,desired_Column], start=c(1), end=c(Training_Week,Training_Day), freq=7)
  bl_test_data <- window(Buy_List_ts_final[,desired_Column], start=c(Test_Week,Test_Day),freq=7)
  bl_data_all <- window(Buy_List_ts_final[,desired_Column], start=c(1),freq=26)
...
mybq <- bq_table(project = "gaeas-cradle", dataset = "buylist_time_series", table = paste(Title_Date,"_BL_FORECAST",sep=""))

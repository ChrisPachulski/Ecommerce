...
setwd("/home/cujo253/Metrics/TBD Updated Roster")
csvFileName <- paste("3_Vendor_History",".csv",sep="")
write.csv(New_Roster, file=csvFileName, row.names = FALSE) 
#Three_Week_Movers <- paste("/home/cujo253/Reports/KPI/Master/",currentDate,"_Master_KPI.csv",sep ="")
Current_Buylist_Tracker <- paste("/home/cujo253/Metrics/TBD Updated Roster/3_Vendor_History.csv", sep ="")
#Make it automate-able
...
data <- data[which(data$Rarity != "C"),]
data <- data[-84]
data <- data[which(data[(ncol(data)-1)] != 0),]
...
# C20_Updated_Roster <- New_Roster
# ncol(New_Roster)
# colnames(New_Roster) <- c("Key","Name","Set","Rarity","Foil",format(seq(from = as.Date("2020-02-29"), to = currentDate, by = 'day'),format = "%Y-%m-%d") )
# New_Roster$Foil <- as.character(New_Roster$Foil)
# 
# New_Roster$Foil[is.na(New_Roster$Foil)==T] <- ""
# #New_Roster[is.na(New_Roster)==T] <- 0
...
# Recombined_data <- rbind(headers, meaned_data)
# Recombined_data <- as.data.frame(Recombined_data, stringsAsFactors = FALSE)
# Recombined_data <- Recombined_data[,colSums(is.na(Recombined_data))<(nrow(Recombined_data)-round(Number_Of_Files*.20,0))]
...
# #Recombined_data <- unique(Recombined_data)
# 
# Cleaned_Recombined_data <- Recombined_data
# #Rejoined headers with numeric Values
# #Cleaned_Recombined_data <- Filter(function(Recombined_data) !any(Recombined_data=="NaN"), Recombined_data)
# #Removed any lingering NaN's (potential for Inf & -Inf that would also need to be addressed should they arise)
# names(Cleaned_Recombined_data) <- as.matrix(Cleaned_Recombined_data[1,])
...
# Titless_Recombined_data <- Cleaned_Recombined_data[-c(1:5),]
# ncol(Cleaned_Recombined_data)
# nrow(Titless_Recombined_data)
# #View(Titless_Recombined_data)
# #Once again, ax those headers for future ts conversion
# values = seq(from = as.Date("2020-02-29"), to = currentDate, by = 'day')
# length(values)
# Titless_Recombined_data$Dates <- values
# col_idx <- grep("Dates", names(Titless_Recombined_data))
# Titless_Recombined_data <- Titless_Recombined_data[, c(col_idx, (1:ncol(Titless_Recombined_data))[-col_idx])]
...
# currentDate <- Sys.Date()
# 
# Time_Series_Training_Dates <- read_csv("/home/cujo253/cronR/KPI_Time_Series_Training_Dates.csv")
# Time_Series_Training_Dates$Date <- as.Date(Time_Series_Training_Dates$Date,"%m/%d/%Y")
# Training_Week <- Time_Series_Training_Dates$Training_Week[match(currentDate,Time_Series_Training_Dates$Date)]
# Training_Day <- Time_Series_Training_Dates$Training_Day[match(currentDate,Time_Series_Training_Dates$Date)]
# Test_Week <- Time_Series_Training_Dates$Test_Week[match(currentDate,Time_Series_Training_Dates$Date)]
# Test_Day <- Time_Series_Training_Dates$Test_Day[match(currentDate,Time_Series_Training_Dates$Date)]
...
# 
# 
# setwd("/home/cujo253/Metrics/Time_Series_Results/KPI")
# csvFileName <- paste(currentDate,"_Times_Series",".csv",sep="")
# write.csv(Final_Compilation, file=csvFileName, row.names = FALSE)
# 
# library(devtools)
...
#After Hours####
ss <- drive_get("KPI_Time_Series")
kpi_ts <- range_read(ss,"Decline")
ss <- drive_get("Vendor_Time_Series_Results")
vendor_ts <- range_read(ss,"Decline")
ss <- drive_get("Time_Series_Results")
bl_ts <- range_read(ss,"All_Cards")

kpi_ts <- kpi_ts[which(kpi_ts$Best_RMSE <= 25),]
kpi_ts <- kpi_ts[which(kpi_ts$Todays_BL <= 1000),]

vendor_ts <- vendor_ts[which(vendor_ts$Todays_BL <= 175),]

Key_Amalgamation <- NULL
Key_Amalgamation <- cbind(kpi_ts$Key, vendor_ts$Key)
Key_Amalgamation <- cbind(Key_Amalgamation, bl_ts$Key)
...
unique_Keys$name <- Updated_Tracking_Keys$name[match(unique_Keys$unique_Keys,Updated_Tracking_Keys$Key)] 
unique_Keys$Set <- Updated_Tracking_Keys$Set[match(unique_Keys$unique_Keys,Updated_Tracking_Keys$Key)]
unique_Keys$Rarity <- Updated_Tracking_Keys$Rarity[match(unique_Keys$unique_Keys,Updated_Tracking_Keys$Key)]

unique_Keys$current_bl <- bl_ts$Todays_BL[match(unique_Keys$unique_Keys,bl_ts$Key)]
unique_Keys$bl_forecast <- bl_ts$Day_7[match(unique_Keys$unique_Keys,bl_ts$Key)]
unique_Keys$bl_RMSE <- bl_ts$Best_RMSE[match(unique_Keys$unique_Keys,bl_ts$Key)]

unique_Keys$current_vendor <- round(vendor_ts$Todays_BL[match(unique_Keys$unique_Keys,vendor_ts$Key)],0)
unique_Keys$vendor_forecast <- round(vendor_ts$Day_7[match(unique_Keys$unique_Keys,vendor_ts$Key)],0)
unique_Keys$vendor_RMSE <- vendor_ts$Best_RMSE[match(unique_Keys$unique_Keys,vendor_ts$Key)]

unique_Keys$current_kpi <- round(kpi_ts$Todays_BL[match(unique_Keys$unique_Keys,kpi_ts$Key)],0)
unique_Keys$kpi_forecast <- round(kpi_ts$Day_7[match(unique_Keys$unique_Keys,kpi_ts$Key)],0)
unique_Keys$kpi_RMSE <- kpi_ts$Best_RMSE[match(unique_Keys$unique_Keys,kpi_ts$Key)]
unique_Keys <- na.omit(unique_Keys)
unique_Keys <- unique_Keys[order(unique_Keys$current_kpi),]

ss <- drive_get("Combined_TS")
sheet_write(
  unique_Keys,
  ss = ss,
  sheet = "Spec_List"
...
gs4_auth(email = "pachun95@gmail.com", use_oob = T)
ss <- drive_get("Time_Series_Results")
BL <- range_read(ss,sheet = "Growth")
ss <- drive_get("Vendor_Time_Series_Results")
Vendors <- range_read(ss, sheet = "Decline")
ss <- drive_get("KPI_Time_Series")
KPI <- range_read(ss, sheet = "Decline")
...
# KPI_TOC$Weeks_Ago <- round(KPI_Delay_Fourteen$Todays_BL[match(KPI_TOC$Key,KPI_Delay_Fourteen$Key)],0)
# KPI_TOC$Weeks_Ago_Forecast <- round(KPI_Delay_Fourteen$Day_7[match(KPI_TOC$Key,KPI_Delay_Fourteen$Key)],0)
# KPI_TOC$Plus_Minus_WA <- round(KPI_Delay_Fourteen$Best_RMSE[match(KPI_TOC$Key,KPI_Delay_Fourteen$Key)],0)
# KPI_TOC$Today <- round(KPI$Todays_BL[match(KPI_TOC$Key,KPI$Key)],0)
# KPI_TOC$Current_Fore <- round(KPI$Day_7[match(KPI_TOC$Key,KPI$Key)],0)
# KPI_TOC$Plus_Minus <- round(KPI$Best_RMSE[match(KPI_TOC$Key,KPI$Key)],0)
# KPI_TOC$Diff <- round(KPI_TOC$Today - KPI_TOC$Weeks_Ago,2)
# KPI_TOC <- KPI_TOC[order(KPI_TOC$Diff),]
# 
# KPI_TOC <- unique(KPI_TOC)
# ss <- drive_get("TS_Delay")
# sheet_write(
#   KPI_TOC,
#   ss = ss,

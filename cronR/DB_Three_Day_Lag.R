...
library(bigrquery)
library(jsonlite)
mtgjson_update <- function(ip){
    con = dbConnect(MySQL(), user='remote', password='zachIsTheBest404!!', dbname ='mtgjson',host="157.245.255.185")
    content <- fromJSON("https://mtgjson.com/api/v5/AtomicCards.json")
    
    data <- Flattener(data.frame(content$data), T)
    str(content$data)
    dbWriteTable(conn=con, overwrite = TRUE, append= FALSE, value = , name ='new_card')
}
...
statement <- paste("SELECT DISTINCT a.Key as Unique_Keys, r.card as Name,r.set as `Set`,r.rarity as Rarity, a.Foil_Status as `F_NF`, a.BL_QTY,a.BL,a.TCG_MKT,a.CK_MKT,a.MKT_Diff,a.Sellers,a.CK_Backing,a.TCG_Backing,a.Group,p.TCG_Rank,p.CK_ADJ_Rank ",
...
statement <- paste("SELECT DISTINCT a.Key as Key, r.card as Name,r.set as `Set`,r.rarity as Rarity,r.hasFoil as `F_NF`,a.Ranking, p.MKT as Retail,p.BL as Buylist ", 
...
KPI_Master <- dbSendQuery(con, statement = statement) %>% dbFetch( n = -1)
...
gs4_auth(email = "pachun95@gmail.com", use_oob=TRUE)
ss <- drive_get("currentDate")
KPI_Table <- range_read(ss,"KPI_Master") %>% filter(is.na(`F/NF`)) %>% 
...
KPI_Table <- KPI_Table %>% mutate(Ranking = seq(nrow(KPI_Table)))  %>%
    select(uuid,Ranking,Retail,Buylist)
...
BL_levels <- range_read(ss,"Buylist") %>% filter(is.na(Foil)) 
BL_levels %>% mutate(uuid = Updated_Tracking_Keys$uuid[match(Unique_Keys,Updated_Tracking_Keys$Key)]) %>%
    select(!c("Unique_Keys","Name","Set","Rarity","Foil","Buylist_Backing")) %>% select(uuid,everything())
dbWriteTable(conn=con, overwrite = TRUE, append= FALSE, value = BL_levels, name ='buylist_levels')

...
timeframe <- gsub("-","_",(currentDate - 90))
...
options(scipen = 20)
statement <- paste(
  "SELECT a.Key,a.Card,a.Set,a.Rarity,a.Foil_Status,a.Sellers,a.Date ",
  "FROM `gaeas-cradle.premiums.*` a ",
  'WHERE Foil_Status not like "%FOIL%" and (Rarity like "R" or Rarity like "M" or Rarity like "U") and a.Set is not NULL and param is not NULL and Sellers is not NULL ',
...
raw_query <- as.data.frame(unique(data))
#raw_query[which(raw_query$Card == "Asceticism" & raw_query$Set == "Mystery Booster"),]
#View(raw_query)
currentDate <- Sys.Date()
timeframe <- (currentDate - 90)
raw_query_base <- data.frame(Key = unique(raw_query$Key))
raw_query_base$Card <- raw_query$Card[match(raw_query_base$Key,raw_query$Key)]
raw_query_base$Set <- raw_query$Set[match(raw_query_base$Key,raw_query$Key)]
raw_query_base$Rarity <- raw_query$Rarity[match(raw_query_base$Key,raw_query$Key)]
raw_query_base$Foil_Status <- raw_query$Foil_Status[match(raw_query_base$Key,raw_query$Key)]
raw_query_day_one <- raw_query[which(raw_query$Date == timeframe),]
raw_query_base$day <- raw_query_day_one$Sellers[match(raw_query_base$Key,raw_query_day_one$Key)]
colnames(raw_query_base)[ncol(raw_query_base)] <- format(as.Date(timeframe), "%m/%d/%Y")

timeframe <- as.Date(timeframe) + 1
for (i in 1:90){
  rqb <- raw_query[which(raw_query$Date == timeframe),]
  raw_query_base$day <- rqb$Sellers[match(raw_query_base$Key,rqb$Key)]
  colnames(raw_query_base)[ncol(raw_query_base)] <- format(as.Date(timeframe), "%m/%d/%Y")
  timeframe <- as.Date(timeframe) + 1
}

new_data <- raw_query_base
...
new_data$Exclude <- Exclusion$Excl_Excl[match(new_data$Set, Exclusion$Set_Excl)]
new_data$Exclude[is.na(new_data$Exclude)] <- "Unclear"
new_data <- new_data[which(new_data$Exclude != "Exclude"),]
new_data <- new_data[-ncol(new_data)]
#summary(new_data)
new_data$Foil_Status <- ifelse(new_data$Foil_Status == "", "NF", new_data$Foil_Status)


trans_data <- t(new_data)
...
meaned_data_1 <- round(na.aggregate(meaned_data),0)
meaned_data_1 <- meaned_data_1[-c(85:90),]

meaned_data_2 <- round(na.aggregate(meaned_data[c(85:90),]),0)
meaned_data <- rbind(meaned_data_1,meaned_data_2)
...
#Cleaned_Recombined_data <- Recombined_data[which(Recombined_data != "NaN"),]
...
values = seq(from = timeframe, to = currentDate, by = 'day')
...
Time_Series_Training_Dates <- data.frame(Date = format(as.Date(timeframe)+1,"%m/%d/%Y"))
Time_Series_Training_Dates$Training_Week <- 1
Time_Series_Training_Dates$Training_Day <- 1
Time_Series_Training_Dates$Test_Week <- 1
Time_Series_Training_Dates$Test_Days <- 2

Time_Series_Training_Dates_2 <- data.frame(Date = format(as.Date(timeframe)+2,"%m/%d/%Y"))
Time_Series_Training_Dates_2$Training_Week <- 1
Time_Series_Training_Dates_2$Training_Day <- 2
Time_Series_Training_Dates_2$Test_Week <- 1
Time_Series_Training_Dates_2$Test_Days <- 3

Time_Series_Training_Dates <- rbind(Time_Series_Training_Dates,Time_Series_Training_Dates_2)
library("anytime")
for (i in 1:88){
  TSTD <- NULL
  TSTD <- data.frame(Date = (anydate(Time_Series_Training_Dates[c(nrow(Time_Series_Training_Dates)),1])+1) )
  TSTD$Date <- format(TSTD$Date,"%m/%d/%Y")
  TSTD$Training_Week <- ifelse(Time_Series_Training_Dates[c(nrow(Time_Series_Training_Dates)),3]==7, Time_Series_Training_Dates[c(nrow(Time_Series_Training_Dates)),2]+1,Time_Series_Training_Dates[c(nrow(Time_Series_Training_Dates)),2] )
  TSTD$Training_Day <- ifelse(Time_Series_Training_Dates[c(nrow(Time_Series_Training_Dates)),3]==7, 1,Time_Series_Training_Dates[c(nrow(Time_Series_Training_Dates)),3]+1 )
  TSTD$Test_Week <- ifelse(Time_Series_Training_Dates[c(nrow(Time_Series_Training_Dates)),5]==7, Time_Series_Training_Dates[c(nrow(Time_Series_Training_Dates)),4]+1,Time_Series_Training_Dates[c(nrow(Time_Series_Training_Dates)),4] )
  TSTD$Test_Days <- ifelse(Time_Series_Training_Dates[c(nrow(Time_Series_Training_Dates)),5]==7, 1,Time_Series_Training_Dates[c(nrow(Time_Series_Training_Dates)),5]+1 )
  Time_Series_Training_Dates <- rbind(Time_Series_Training_Dates,TSTD)
}
Time_Series_Training_Dates$Date <- format(anydate(Time_Series_Training_Dates$Date),"%Y-%m-%d")

Training_Week <- Time_Series_Training_Dates$Training_Week[match(currentDate,anydate(Time_Series_Training_Dates$Date))] - 3
Training_Day <- Time_Series_Training_Dates$Training_Day[match((currentDate-26),anydate(Time_Series_Training_Dates$Date))]
Test_Week <- Time_Series_Training_Dates$Test_Week[match(currentDate,anydate(Time_Series_Training_Dates$Date))] - 3
Test_Day <- Time_Series_Training_Dates$Test_Day[match((currentDate-26),anydate(Time_Series_Training_Dates$Date))]
...
for(Key in Reformatted_Keys[c(1:total),]){
  ...
  bl_train_data_hw <- hw(bl_train_data, seasonal = "multiplicative", h = 26)
...
for(Key in Reformatted_Keys[c(1:total),]){
  ...
  nnetforecast <- forecast(fit, h=26, PI = F)
  ...
  Vec2<- (cbind(bl_test_data, as.data.frame(forecast(fit, h=26, PI = F))[,1]))
...
for(Key in Reformatted_Keys[c(1:total),]){
  ...
  MyArima_Forecast <- forecast(MyArima, h = 26)
  training_guardrails <- as.data.frame(bl_train_data)
  Arima_Vector <- cbind(bl_test_data,training_guardrails[c((nrow(training_guardrails)-25):nrow(training_guardrails)),1])
...
Final_Compilation$HW_RMSE <- round(Filtered_Angle[,14][match(Final_Compilation$Key,Filtered_Angle$Key)],0)
Final_Compilation$NN_RMSE <- round(NN_Filtered_Angle[,14][match(Final_Compilation$Key,NN_Filtered_Angle$Key)],0)
Final_Compilation$ARIMA_RMSE <- round(Filtered_Angle_ARIMA[,14][match(Final_Compilation$Key,Filtered_Angle_ARIMA$Key)],0)
...
Final_Compilation$Chosen_Model <- ifelse((Final_Compilation$HW_RMSE <= Final_Compilation$NN_RMSE & Final_Compilation$HW_RMSE <= Final_Compilation$ARIMA_RMSE), "Holts-Winter",
                                         ifelse((Final_Compilation$NN_RMSE <= Final_Compilation$HW_RMSE & Final_Compilation$NN_RMSE <= Final_Compilation$ARIMA_RMSE), "Neural Net",
                                                ifelse((Final_Compilation$ARIMA_RMSE <= Final_Compilation$HW_RMSE & Final_Compilation$ARIMA_RMSE <= Final_Compilation$NN_RMSE),"ARIMA",
                                                       "Game Over Man!")))
Final_Compilation$Day_1 <- ifelse(Final_Compilation$Chosen_Model == "Holts-Winter", round(Filtered_Angle[,7][match(Final_Compilation$Key,Filtered_Angle$Key)],0),
                                  ifelse(Final_Compilation$Chosen_Model == "Neural Net", round(NN_Filtered_Angle[,7][match(Final_Compilation$Key,NN_Filtered_Angle$Key)],0),
                                         ifelse(Final_Compilation$Chosen_Model == "ARIMA", round(Filtered_Angle_ARIMA[,7][match(Final_Compilation$Key,Filtered_Angle_ARIMA$Key)],0),
                                                "Game Over Man!")))

Final_Compilation$Day_2 <- ifelse(Final_Compilation$Chosen_Model == "Holts-Winter", round(Filtered_Angle[,8][match(Final_Compilation$Key,Filtered_Angle$Key)],0),
                                  ifelse(Final_Compilation$Chosen_Model == "Neural Net", round(NN_Filtered_Angle[,8][match(Final_Compilation$Key,NN_Filtered_Angle$Key)],0),
                                         ifelse(Final_Compilation$Chosen_Model == "ARIMA", round(Filtered_Angle_ARIMA[,8][match(Final_Compilation$Key,Filtered_Angle_ARIMA$Key)],0),
                                                "Game Over Man!")))
Final_Compilation$Day_3 <- ifelse(Final_Compilation$Chosen_Model == "Holts-Winter", round(Filtered_Angle[,9][match(Final_Compilation$Key,Filtered_Angle$Key)],0),
                                  ifelse(Final_Compilation$Chosen_Model == "Neural Net", round(NN_Filtered_Angle[,9][match(Final_Compilation$Key,NN_Filtered_Angle$Key)],0),
                                         ifelse(Final_Compilation$Chosen_Model == "ARIMA", round(Filtered_Angle_ARIMA[,9][match(Final_Compilation$Key,Filtered_Angle_ARIMA$Key)],0),
                                                "Game Over Man!")))
Final_Compilation$Day_4 <- ifelse(Final_Compilation$Chosen_Model == "Holts-Winter", round(Filtered_Angle[,10][match(Final_Compilation$Key,Filtered_Angle$Key)],0),
                                  ifelse(Final_Compilation$Chosen_Model == "Neural Net", round(NN_Filtered_Angle[,10][match(Final_Compilation$Key,NN_Filtered_Angle$Key)],0),
                                         ifelse(Final_Compilation$Chosen_Model == "ARIMA", round(Filtered_Angle_ARIMA[,10][match(Final_Compilation$Key,Filtered_Angle_ARIMA$Key)],0),
                                                "Game Over Man!")))
Final_Compilation$Day_5 <- ifelse(Final_Compilation$Chosen_Model == "Holts-Winter", round(Filtered_Angle[,11][match(Final_Compilation$Key,Filtered_Angle$Key)],0),
                                  ifelse(Final_Compilation$Chosen_Model == "Neural Net", round(NN_Filtered_Angle[,11][match(Final_Compilation$Key,NN_Filtered_Angle$Key)],0),
                                         ifelse(Final_Compilation$Chosen_Model == "ARIMA", round(Filtered_Angle_ARIMA[,11][match(Final_Compilation$Key,Filtered_Angle_ARIMA$Key)],0),
                                                "Game Over Man!")))
Final_Compilation$Day_6 <- ifelse(Final_Compilation$Chosen_Model == "Holts-Winter", round(Filtered_Angle[,12][match(Final_Compilation$Key,Filtered_Angle$Key)],0),
                                  ifelse(Final_Compilation$Chosen_Model == "Neural Net", round(NN_Filtered_Angle[,12][match(Final_Compilation$Key,NN_Filtered_Angle$Key)],0),
                                         ifelse(Final_Compilation$Chosen_Model == "ARIMA", round(Filtered_Angle_ARIMA[,12][match(Final_Compilation$Key,Filtered_Angle_ARIMA$Key)],0),
                                                "Game Over Man!")))
Final_Compilation$Day_7 <- ifelse(Final_Compilation$Chosen_Model == "Holts-Winter", round(Filtered_Angle[,13][match(Final_Compilation$Key,Filtered_Angle$Key)],0),
                                  ifelse(Final_Compilation$Chosen_Model == "Neural Net", round(NN_Filtered_Angle[,13][match(Final_Compilation$Key,NN_Filtered_Angle$Key)],0),
                                         ifelse(Final_Compilation$Chosen_Model == "ARIMA", round(Filtered_Angle_ARIMA[,13][match(Final_Compilation$Key,Filtered_Angle_ARIMA$Key)],0),
                                                "Game Over Man!")))
Final_Compilation$Best_RMSE <- ifelse(Final_Compilation$Chosen_Model == "Holts-Winter", round(Filtered_Angle[,14][match(Final_Compilation$Key,Filtered_Angle$Key)],0),
                                      ifelse(Final_Compilation$Chosen_Model == "Neural Net", round(NN_Filtered_Angle[,14][match(Final_Compilation$Key,NN_Filtered_Angle$Key)],0),
                                             ifelse(Final_Compilation$Chosen_Model == "ARIMA", round(Filtered_Angle_ARIMA[,14][match(Final_Compilation$Key,Filtered_Angle_ARIMA$Key)],0),
                                                    "Game Over Man!")))
Final_Compilation$Best_MAPE <- ifelse(Final_Compilation$Chosen_Model == "Holts-Winter", Filtered_Angle[,15][match(Final_Compilation$Key,Filtered_Angle$Key)],
                                      ifelse(Final_Compilation$Chosen_Model == "Neural Net", NN_Filtered_Angle[,15][match(Final_Compilation$Key,NN_Filtered_Angle$Key)],
                                             ifelse(Final_Compilation$Chosen_Model == "ARIMA", Filtered_Angle_ARIMA[,15][match(Final_Compilation$Key,Filtered_Angle_ARIMA$Key)],
                                                    "Game Over Man!")))
Final_Compilation$Best_Diff <- ifelse(Final_Compilation$Chosen_Model == "Holts-Winter", round(Filtered_Angle[,16][match(Final_Compilation$Key,Filtered_Angle$Key)],0),
                                      ifelse(Final_Compilation$Chosen_Model == "Neural Net", round(NN_Filtered_Angle[,16][match(Final_Compilation$Key,NN_Filtered_Angle$Key)],0),
                                             ifelse(Final_Compilation$Chosen_Model == "ARIMA", round(Filtered_Angle_ARIMA[,16][match(Final_Compilation$Key,Filtered_Angle_ARIMA$Key)],0),
                                                    "Game Over Man!")))
...
Final_Compilation <- Final_Compilation[which(Final_Compilation$Best_RMSE> (-25) & Final_Compilation$Best_RMSE< (25)),]
...
Confidant_Volatility <- Final_Compilation[which(Final_Compilation$RMSE_Pct_Volatility < (RMSE_Cutoff)),]
...
Volatile_Decline <- Final_Compilation[which(Final_Compilation$Dollar_Pct_Chg < 0),]
Volatile_Decline <- Volatile_Decline[order(Volatile_Decline$Dollar_Pct_Chg),]
nrow(Volatile_Decline)

setwd("/home/cujo253/Metrics/Time_Series_Results/Vendor")
csvFileName <- paste(currentDate,"_Vendor_Times_Series",".csv",sep="")
...
Final_Compilation_Export <- tryCatch({Final_Compilation %>% rename(c(`F/NF` = "Foil")) %>% mutate(uuid = Updated_Tracking_Keys$uuid[match(Key,Updated_Tracking_Keys$Key)], Date = Sys.Date()) %>% select(uuid,everything())}, error = function(e){Final_Compilation %>% rename(c("Foil" = `F/NF`)) %>% mutate(uuid = Updated_Tracking_Keys$uuid[match(Key,Updated_Tracking_Keys$Key)], Date = Sys.Date()) %>% select(uuid,everything())})
...
bq_table_upload(x=mybq, values = Final_Compilation_Export, fields=as_bq_fields(Final_Compilation_Export),nskip = 1, source_format = "CSV",create_disposition = "CREATE_IF_NEEDED", write_disposition = "WRITE_APPEND")
...
BL <- drive_get("Time_Series_Results") %>% range_read(sheet = "Growth") %>% as.data.frame()
Vendors <- drive_get("Vendor_Time_Series_Results") %>% range_read(sheet = "Decline") %>% as.data.frame()


BL_TOC <- BL[1:5]
Vendors_TOC <- Vendors[1:5]


DelayDate_1 <- currentDate - 6
statement <- paste("SELECT *   ","FROM `gaeas-cradle.buylist_time_series.",gsub("-","_",DelayDate_1),"_BL_FORECAST` a ",sep = "")
BL_Delay_Fourteen <- dbSendQuery(con, statement = statement) %>% dbFetch(n = -1) %>% select(!c("uuid","Date"))


BL_TOC$Weeks_Ago <- round(BL_Delay_Fourteen$Todays_BL[match(BL_TOC$Key,BL_Delay_Fourteen$Key)],0)
BL_TOC$Weeks_Ago_Forecast <- round(BL_Delay_Fourteen$Day_7[match(BL_TOC$Key,BL_Delay_Fourteen$Key)],0)
BL_TOC$Plus_Minus_WA <- round(BL_Delay_Fourteen$Best_RMSE[match(BL_TOC$Key,BL_Delay_Fourteen$Key)],0)
BL_TOC$Today <- round(BL$Todays_BL[match(BL_TOC$Key,BL$Key)],0)
BL_TOC$Current_Fore <- round(BL$Day_7[match(BL_TOC$Key,BL$Key)],0)
BL_TOC$Plus_Minus <- round(BL$Best_RMSE[match(BL_TOC$Key,BL$Key)],0)
BL_TOC$Diff <- round(BL_TOC$Today - BL_TOC$Weeks_Ago,2)
BL_TOC <- BL_TOC[order(-BL_TOC$Diff),]
BL_TOC <- BL_TOC[which(BL_TOC$Weeks_Ago > 0),]
BL_TOC <- unique(BL_TOC)
...
statement <- paste("SELECT *   ","FROM `gaeas-cradle.vendor_time_series.",gsub("-","_",DelayDate_1),"_VENDOR_FORECAST` a ",sep = "")
Vendor_Delay_Fourteen <- dbSendQuery(con, statement = statement) %>% dbFetch(n = -1) %>% select(!c("uuid","Date"))

Vendors_TOC$Weeks_Ago <- round(Vendor_Delay_Fourteen$Todays_BL[match(Vendors_TOC$Key,Vendor_Delay_Fourteen$Key)],0)
Vendors_TOC$Weeks_Ago_Forecast <- round(Vendor_Delay_Fourteen$Day_7[match(Vendors_TOC$Key,Vendor_Delay_Fourteen$Key)],0)
Vendors_TOC$Plus_Minus_WA <- round(Vendor_Delay_Fourteen$Best_RMSE[match(Vendors_TOC$Key,Vendor_Delay_Fourteen$Key)],0)
Vendors_TOC$Today <- round(Vendors$Todays_BL[match(Vendors_TOC$Key,Vendors$Key)],0)
Vendors_TOC$Current_Fore <- round(Vendors$Day_7[match(Vendors_TOC$Key,Vendors$Key)],0)
Vendors_TOC$Plus_Minus <- round(Vendors$Best_RMSE[match(Vendors_TOC$Key,Vendors$Key)],0)
Vendors_TOC$Diff <- round(Vendors_TOC$Today - Vendors_TOC$Weeks_Ago,2)
Vendors_TOC <- Vendors_TOC[order(Vendors_TOC$Diff),]
Vendors_TOC <- unique(Vendors_TOC)
ss <- drive_get("TS_Delay")
sheet_write(
  Vendors_TOC,
  ss = ss,
...
DelayDate_1 <- currentDate - 2
statement <- paste("SELECT *   ","FROM `gaeas-cradle.buylist_time_series.",gsub("-","_",DelayDate_1),"_BL_FORECAST` a ",sep = "")
BL <- dbSendQuery(con, statement = statement) %>% dbFetch(n = -1) %>% select(!c("uuid","Date")) %>% distinct()

RMSE_Cutoff <- mean(BL$RMSE_Pct_Volatility)
Dollar_Cutoff <- mean(BL$Dollar_Pct_Chg)

Confidant_Volatility <- BL[which(BL$RMSE_Pct_Volatility < (RMSE_Cutoff)),]
Confidant_Volatility <- Confidant_Volatility[which(Confidant_Volatility$Dollar_Pct_Chg > (Dollar_Cutoff * 2) | Confidant_Volatility$Dollar_Pct_Chg > ((Dollar_Cutoff*-1) * 2)),]
Confidant_Volatility <- Confidant_Volatility[which(Confidant_Volatility$Dollar_Pct_Chg != 0),]
Confidant_Volatility <- Confidant_Volatility[order(-Confidant_Volatility$Dollar_Pct_Chg),]
...
statement <- paste("SELECT *   ","FROM `gaeas-cradle.vendor_time_series.",gsub("-","_",DelayDate_1),"_VENDOR_FORECAST` a ",sep = "")
Vendor <- dbSendQuery(con, statement = statement) %>% dbFetch(n = -1) %>% select(!c("uuid","Date")) %>% distinct()

RMSE_Cutoff <- mean(Vendor$RMSE_Pct_Volatility)
Dollar_Cutoff <- mean(Vendor$Dollar_Pct_Chg)

Confidant_Volatility <- Vendor[which(Vendor$RMSE_Pct_Volatility < (RMSE_Cutoff)),]
Confidant_Volatility <- Confidant_Volatility[which(Confidant_Volatility$Dollar_Pct_Chg > (Dollar_Cutoff * 2) | Confidant_Volatility$Dollar_Pct_Chg > ((Dollar_Cutoff*-1) * 2)),]
Confidant_Volatility <- Confidant_Volatility[which(Confidant_Volatility$Dollar_Pct_Chg != 0),]
Confidant_Volatility <- Confidant_Volatility[order(-Confidant_Volatility$Dollar_Pct_Chg),]
...
DelayDate_1 <- currentDate - 9

statement <- paste("SELECT *   ","FROM `gaeas-cradle.buylist_time_series.",gsub("-","_",DelayDate_1),"_BL_FORECAST` a ",sep = "")
BL_Delay_Fourteen <- dbSendQuery(con, statement = statement) %>% dbFetch(n = -1) %>% select(!c("uuid","Date")) %>% distinct()
...
statement <- paste("SELECT *   ","FROM `gaeas-cradle.vendor_time_series.",gsub("-","_",DelayDate_1),"_VENDOR_FORECAST` a ",sep = "")
Vendor_Delay_Fourteen <- dbSendQuery(con, statement = statement) %>% dbFetch(n = -1) %>% select(!c("uuid","Date")) %>% distinct()
...
# DelayDate_1 <- currentDate - 7
# BL <- read_csv(paste("/home/cujo253/Metrics/Time_Series_Results/Buylist/",DelayDate_1,"_Times_Series.csv",sep=""))
# 
# Vendor <- read_csv(paste("/home/cujo253/Metrics/Time_Series_Results/Vendor/",DelayDate_1,"_Vendor_Times_Series.csv",sep=""))
# 
# # KPI <- read_csv(paste("/home/cujo253/Metrics/Time_Series_Results/KPI/",DelayDate_1,"_Times_Series.csv",sep=""))
# 
# BL_TOC <- BL[1:5]
# Vendors_TOC <- Vendors[1:5]
# # KPI_TOC <- KPI[1:5]
# 
# DelayDate_1 <- currentDate - 14
# BL_Delay_Fourteen <- read_csv(paste("/home/cujo253/Metrics/Time_Series_Results/Buylist/",DelayDate_1,"_Times_Series.csv",sep=""))
# Vendor_Delay_Fourteen <- read_csv(paste("/home/cujo253/Metrics/Time_Series_Results/Vendor/",DelayDate_1,"_Vendor_Times_Series.csv",sep=""))
# # KPI_Delay_Fourteen <- read_csv(paste("/home/cujo253/Metrics/Time_Series_Results/KPI/",DelayDate_1,"_Times_Series.csv",sep=""))
# 
# BL_TOC$Weeks_Ago <- round(BL_Delay_Fourteen$Todays_BL[match(BL_TOC$Key,BL_Delay_Fourteen$Key)],0)
# BL_TOC$Weeks_Ago_Forecast <- round(BL_Delay_Fourteen$Day_7[match(BL_TOC$Key,BL_Delay_Fourteen$Key)],0)
# BL_TOC$Plus_Minus_WA <- round(BL_Delay_Fourteen$Best_RMSE[match(BL_TOC$Key,BL_Delay_Fourteen$Key)],0)

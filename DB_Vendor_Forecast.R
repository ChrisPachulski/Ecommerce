...
new_data[which(new_data$Card == "Goblin Guide"),]
...
meaned_data_1 <- meaned_data_1[-c(87:90),]
meaned_data_2 <- round(na.aggregate(meaned_data[c(87:90),]),0)
...
Final_Compilation_Export <- Final_Compilation %>% rename(c(`F/NF` = "Foil")) %>% mutate(uuid = Updated_Tracking_Keys$uuid[match(Key,Updated_Tracking_Keys$Key)], Date = Sys.Date()) %>% select(uuid,everything())
